...
pub async fn receiver_ssh(
    mut port: thrussh::client::Channel,
    rx:       std::sync::mpsc::Receiver<()>,
    flags:    Arc<Mutex<flag::Flags>>,
    params:   Option<setting::Params>)
{
                ...
                ChannelMsg::Data{data} => {
                    ...
                    if *flags.lock().unwrap().hexdump() {
                        ...
                        if *flags.lock().unwrap().nocolor() {
                            ...
                            color::coloring_words(
                                &String::from_utf8_lossy(&t), &mut last_word, &params);
                        }
                    }
                    ...
                    // Write to log_buf from serial_buf
                    string_from_utf8_appearance(&mut log_buf, &t);
    ...
    // Non save log
        ...
        loop {
            ...
            match port.wait().await.unwrap() {
                ...
                thrussh::ChannelMsg::WindowAdjusted{new_size} => println!("WindowAdjusted: {:x}", new_size),
                thrussh::ChannelMsg::Success => println!("Success"),
                thrussh::ChannelMsg::Eof  => println!("Eof"),
                thrussh::ChannelMsg::Close => println!("Close"),
                // thrussh::ChannelMsg::ExitSignal{} => println!("exitstatus"),
                thrussh::ChannelMsg::ExtendedData{data, ext} => {
                    dbg!("ExtendedData", ext);
                    let t = data.as_ref();
                    if *flags.lock().unwrap().debug() {
                        print!("{:?}", &t);
                    }
...
pub async fn transmitter_ssh(mut port: thrussh::client::Channel, tx: std::sync::mpsc::Sender<()>, flags: Arc<Mutex<flag::Flags>>) {
    use crate::default::escape_sequences::*;
    ...
    loop {
        ...
        match ch {
            Ok(key) => {
                    ...
                    match key {
                        ...
                        COMMAND_1 => {
                            ...
                            if let Some(command) = echo_stdin_read_line() {
                                ...
                                // Send
                                if let Ok(output) = output {
                                    port.data(&output.stdout[..]).await.unwrap();
                                }
                            }
                            continue;
                        },
                ...
                // Send key
                match key {
                    Key::Null      => port.data(&[ 0x00 ][..]).await.unwrap(),
                    Key::Backspace => port.data(&[ 0x08 ][..]).await.unwrap(),
                    Key::Delete    => port.data(&[ 0x7F ][..]).await.unwrap(),
                    Key::Esc       => port.data(&[ 0x1B ][..]).await.unwrap(),
                    Key::Up        => port.data(&[ 0x1B, b'[', b'A' ][..]).await.unwrap(),
                    Key::Down      => port.data(&[ 0x1B, b'[', b'B' ][..]).await.unwrap(),
                    Key::Right     => port.data(&[ 0x1B, b'[', b'C' ][..]).await.unwrap(),
                    Key::Left      => port.data(&[ 0x1B, b'[', b'D' ][..]).await.unwrap(),
                    Key::End       => port.data(&[ 0x1B, b'[', b'F' ][..]).await.unwrap(),
                    Key::Home      => port.data(&[ 0x1B, b'[', b'H' ][..]).await.unwrap(),
                    Key::BackTab   => port.data(&[ 0x1B, b'[', b'Z' ][..]).await.unwrap(),
                    Key::Insert    => port.data(&[ 0x1B, b'[', b'2', b'~' ][..]).await.unwrap(),
                    Key::PageUp    => port.data(&[ 0x1B, b'[', b'5', b'~' ][..]).await.unwrap(),
                    Key::PageDown  => port.data(&[ 0x1B, b'[', b'6', b'~' ][..]).await.unwrap(),
                    Key::F(num) => {
                        match num {
                            v @  1..= 5 => port.data(&[ 0x1B, b'[', b'1', v + b'0',     b'~' ][..]).await.unwrap(),
                            v @  6..= 8 => port.data(&[ 0x1B, b'[', b'1', v + b'0' + 1, b'~' ][..]).await.unwrap(),
                            v @  9..=10 => port.data(&[ 0x1B, b'[', b'2', v + b'0' - 9, b'~' ][..]).await.unwrap(),
                            v @ 11..=12 => port.data(&[ 0x1B, b'[', b'2', v + b'0' - 8, b'~' ][..]).await.unwrap(),
                            _ => unreachable!(),
                        }
                    },
                    Key::Char(ch) => port.data(&ch.encode_utf8(&mut [0; 4]).as_bytes()[..]).await.unwrap(),
                    Key::Alt(ch)  => port.data(&[ 0x1B, ch as u8 ][..]).await.unwrap(),
                    Key::Ctrl(ch) => {
                        match ch {
                            'a'..='z' => port.data(&[ (ch as u8) - b'a' + 1 ][..]).await.unwrap(),
                            '4' => port.data(&[ 0x1B, b'[', b'1', b';', b'5', b'S' ][..]).await.unwrap(),
                            '5' => port.data(&[ 0x1B, b'[', b'1', b'5', b';', b'5', b'~' ][..]).await.unwrap(),
                            '6' => port.data(&[ 0x1B, b'[', b'1', b'7', b';', b'5', b'~' ][..]).await.unwrap(),
                            '7' => port.data(&[ 0x1B, b'[', b'1', b'8', b';', b'5', b'~' ][..]).await.unwrap(),
                            _ => unreachable!(),
                        }
                    },
                    Key::Other(b) => port.data(&b[..]).await.unwrap(),
                }
